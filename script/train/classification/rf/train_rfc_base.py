...
from script.train.classification.rf import train_rfc_base
...
def main(args: List[str]):
    argv: Namespace = train_rfc_base.parse_args(args)
    ...
    dataset_config, mel_spec_config, reshape_config, pool_config, rfc_config, loader_config = train_rfc_base.get_config(
        argv=argv)
    ...
    for curr_val_fold in range(dataset_config.k_folds):
        ...
        rfc: RandomForestClassifier = train_rfc_base.train_rfc(
            curr_val_fold=curr_val_fold,
            dataset=train,
            rfc_config=rfc_config,
            export_path=export_path)
        classify_common.report_slices_acc(classifier=rfc, train=train, val=val)
