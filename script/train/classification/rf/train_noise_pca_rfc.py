...
from script.train.classification.rf import train_noise_pca_rfc, train_pca_rfc
...
def main(args: List[str]):
    argv: Namespace = train_noise_pca_rfc.parse_args(args)
    ...
    dataset_config, sound_wave_augment_config, mel_spec_config, reshape_config, pool_config, pca_config, rfc_config, loader_config = train_noise_pca_rfc.get_config(
        argv=argv)
    ...
    for curr_val_fold in range(dataset_config.k_folds):
        ...
        collate_func: CollateFuncType = collate_base.EnsembleCollateFunction(
            collate_funcs=[
                partial(collate_augment_sound_wave.add_white_noise_collate,
                        config=sound_wave_augment_config),
        ...
        pca_rfc: Pipeline = train_pca_rfc.train_pca_rfc(
            curr_val_fold=curr_val_fold,
            dataset=train,
            pca_config=pca_config,
            rfc_config=rfc_config,
            export_path=export_path)
        classify_common.report_slices_acc(classifier=pca_rfc,
                                          train=train,
                                          val=val)
