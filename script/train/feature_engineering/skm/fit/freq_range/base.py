...
import audio_classifier.config.preprocessing.freq_range as conf_freq_range
...
import audio_classifier.train.collate.preprocessing.spectrogram.freq_range as collate_freq_range
...
def main(args: List[str]):
    ...
    dataset_config, mel_spec_config, freq_range_config, reshape_config, skm_config, loader_config = get_config(
        argv=argv)
    ...
    collate_func: CollateFuncType = collate_base.EnsembleCollateFunction(
        collate_funcs=[
            ...
            partial(collate_freq_range.split_freq_range_spec_collate,
                    config=freq_range_config),
            partial(collate_reshape.slice_flatten_freq_range_collate,
...
def get_argparse() -> ArgumentParser:
    parser = ArgumentParser(parents=[
        ...
        conf_freq_range.FreqRangeArgumentParser(),
...
def get_config(argv: Namespace):
    ...
    FREQ_RANGE_CONFIG_PATH: str = argv.freq_range_config_path
    ...
    freq_range_config: conf_freq_range.FreqRangeConfig = conf_freq_range.get_freq_range_config(
    ...
    loader_config: conf_loader.LoaderConfig = conf_loader.get_loader_config_from_json(
        LOADER_CONFIG_PATH)
    return dataset_config, mel_spec_config, freq_range_config, reshape_config, skm_config, loader_config
