...
import audio_classifier.config.feature_engineering.contrast as conf_contrast
...
import audio_classifier.train.collate.feature_engineering.contrast as collate_contrast
...
def main(args: List[str]):
    ...
    dataset_config, mel_spec_config, reshape_config, contrast_config, skm_config, loader_config = get_config(
        argv=argv)
    ...
    collate_func: CollateFuncType = collate_base.EnsembleCollateFunction(
        collate_funcs=[
            ...
            partial(collate_contrast.contrast_collate,
                    threshold_func=partial(np.quantile, q=0.25),
    ...
    for curr_val_fold in range(dataset_config.k_folds):
        ...
        for curr_label in unique_labels:
            ...
            curr_slices: np.ndarray = fit_skm_base.get_curr_class_slices(
                curr_label, slices, labels)
            k_value: Union[int, None] = fit_skm_base.try_k_elbow(
            ...
            if k_value is None:
                continue
            skm, model_path = fit_skm_base.fit_skm(
                curr_class_path=curr_class_path,
                slices=curr_slices,
                skm_config=skm_config,
                k_value=k_value)
            is_pass: bool = fit_skm_base.verify_model(slices=slices,
                                                      skm=skm,
            ...
            if is_pass == False:
                print("Discrepency between src model and exported model",
                      file=sys.stderr)
                sys.exit(1)
            fit_skm_base.plot_centroids(curr_plot_path=curr_class_path,
                                        skm=skm,
            ...
            fit_skm_base.plot_silhouette(curr_plot_path=curr_class_path,
                                         slices=curr_slices,
                                         skm=skm,
                                         k_value=k_value)
...
def get_argparse() -> ArgumentParser:
    parser = ArgumentParser(parents=[
        ...
        conf_contrast.ContrastConfigArgumentParser(),
...
def get_config(argv: Namespace):
    ...
    CONTRAST_CONFIG_PATH: str = argv.contrast_config_path
    ...
    reshape_config: conf_reshape.ReshapeConfig = conf_reshape.get_reshape_config_from_json(
        RESHAPE_CONFIG_PATH)
    contrast_config: conf_contrast.ContrastConfig = conf_contrast.get_contrast_config_from_json(
    ...
    loader_config: conf_loader.LoaderConfig = conf_loader.get_loader_config_from_json(
        LOADER_CONFIG_PATH)
    return dataset_config, mel_spec_config, reshape_config, contrast_config, skm_config, loader_config
