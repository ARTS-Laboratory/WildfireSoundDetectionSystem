...
#model/baseline/base/spec_00_reshape_00_skm_00/svm/pool_02_pca_00_svc_00
AUDIO_PATH: str = "../test/test_audio/fire_02.wav"
...
VAL_PCA_PATH_STUB: str = "val_pca_{:02d}"
...
def get_pca_path(curr_val_fold: int,
                 pca_model_root_path: str,
                 val_path_stub: str,
                 file_ext: str = ".pkl"):
    pca_path_stub: str = os.path.join(pca_model_root_path,
                                      val_path_stub + file_ext)
    pca_path = str.format(pca_path_stub, curr_val_fold)
    return pca_path
...
def get_pca(pca_path: str) -> PCA:
    with open(pca_path, "rb") as pca_file:
        pca: PCA = pickle.load(pca_file)
        return pca


#%%
CURR_VAL_PCA_PATH: str = get_pca_path(curr_val_fold=CURR_VAL_FOLD,
                                      pca_model_root_path=SVM_MODEL_ROOT_PATH,
                                      val_path_stub=VAL_PCA_PATH_STUB)
pca = get_pca(CURR_VAL_PCA_PATH)

#%%
pca_out: np.ndarray = pca.transform(np.asarray(pool_slices))
